...
class Deadline {
public:
    explicit Deadline(std::chrono::milliseconds timeout = std::chrono::seconds(30))
        : m_Watcher([timeout](std::stop_token token) {
              while (!token.stop_requested() && !isRequested())
                  std::this_thread::sleep_for(std::chrono::milliseconds(10));
              const auto end = std::chrono::steady_clock::now() + timeout;
              while (!token.stop_requested()) {
                  if (std::chrono::steady_clock::now() >= end) {
                      constexpr char message[] = "gameserver shutdown deadline exceeded; forcing process exit\n";
