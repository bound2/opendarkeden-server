...

{
    ...
    // Compare the level. The comparison is skipped for a Slayer, which passes
    // unconditionally.
    if (m_ReqLevel > 0) {
        if (pCreature2->isSlayer()) {
            return true;
    ...
    try {
        ...
        while (d < text.size() - 1) {
            ...
            if (attr == "STR") {
                ...
                throw Error("ConditionAttrComp::read() : Unknown attribute type");
            }
...

{
    ...
    if (op == "LOWER")
        ...
        throw Error("ConditionAttrComp::parseOperator() : Unknown operator");
    }
