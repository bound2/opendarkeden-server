...
bool MeetNPCQuestStatus::isTarget(NPCID_t npcID) const {
    if (m_State != QUEST_STARTED)
        return false;
    if (!m_bMet[0])
        return m_TargetNPCID[0] == npcID;
    if (!m_bMet[1])
        return m_TargetNPCID[1] == npcID;
    return false;
}
...
bool MeetNPCQuestStatus::met(NPCID_t npcID) {
    if (m_State != QUEST_STARTED)
        return false;
    if (!m_bMet[0])
        return (m_TargetNPCID[0] == npcID) ? (m_bMet[0] = true) : false;
    if (!m_bMet[1])
        return (m_TargetNPCID[1] == npcID) ? (m_bMet[1] = true) : false;
    return false;
}
...
NPCID_t MeetNPCQuestStatus::getTargetID() const {
    if (m_State != QUEST_STARTED)
        return 0;
    if (!m_bMet[0])
        return m_TargetNPCID[0];
    if (!m_bMet[1])
        return m_TargetNPCID[1];
    return 0;
}
