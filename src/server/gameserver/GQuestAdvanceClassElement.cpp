...
GQuestElement::ResultType GQuestAdvanceClassElement::checkCondition(PlayerCreature* pPC) const {
    ...
    if (pPC->isSlayer()) {
        ...
        DomainInfo* pDomainInfo = de::gameContext().skillDomains().getDomainInfo((SkillDomain)domain, pPC->getLevel());
        if (pDomainInfo == NULL)
            return FAIL;
        ...
        VampEXPInfo* pExpInfo = de::gameContext().vampireExp().getVampEXPInfo(pVampire->getLevel());
        if (pExpInfo == NULL)
            return FAIL;
        ...
        OustersEXPInfo* pExpInfo = de::gameContext().oustersExp().getOustersEXPInfo(pOusters->getLevel());
        if (pExpInfo == NULL)
            return FAIL;
        Exp_t goalExp = pExpInfo->getGoalExp();
