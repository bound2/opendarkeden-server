...
public:
    ...
    static bool markListingSold(int64_t listingID, const string& buyerAccount, const string& buyerPlayer);

    // Query listings with pagination and filters
    static vector<ExchangeListing> getListings(int16_t serverID, uint8_t status, int page, int pageSize);

    // Get a specific listing by ID
    static ExchangeListing* getListing(int64_t listingID);

    // Get seller's active listings
    static vector<ExchangeListing> getSellerListings(const string& sellerAccount, uint8_t status);
    ...
    // Get buyer's orders
    static vector<ExchangeOrder> getBuyerOrders(const string& buyerPlayer, uint8_t status);

    // Get seller's fulfilled orders
    static vector<ExchangeOrder> getSellerOrders(const string& sellerPlayer, uint8_t status);
    ...
    static bool adjustPoints(const string& account, int delta, int& balanceAfter, uint8_t reason, int64_t refListingID,
