...
#include "BombCrashWalk.h"     // Giant cannon bombardment
#include "HeavenGround.h"      // Descent of the heavenly god
#include "IllusionInversion.h" // Terror illusion (Slayer doctor)
#include "SatelliteBomb.h"     // Satellite bombardment (new Slayer gun skill)
#include "ShineSword.h"        // Shining sword (Slayer sword skill)
// Ousters
#include "BigRockfall.h"      //395 ADD BY RALLSER
#include "BrambleHalo.h"      //390 ADD BY RALLSER
#include "CutStorm.h"         //387 ADD BY RALLSER
#include "DeadlyClaw.h"       // 391 wounding rend claw
#include "DummyDrake.h"       // Drake puppet (fire mage)
#include "FireMeteor.h"       //394 ADD BY RALLSER
#include "HeterChakram.h"     // Heter chakram (agility fighter)
#include "HydroConvergence.h" // Compound water cure (water mage)
...
#include "SkyFire.h"          // 386 sky thunder fire
#include "SummonClay.h"       // Clay summon (earth mage)
...
SkillHandlerManager::SkillHandlerManager() : m_SkillHandlers(NULL), m_Size(SKILL_MAX) {
    ...
    // Create the skill handler array.
    m_SkillHandlers = new SkillHandler*[m_Size];

    // Initialize the skill handler array.
...
SkillHandlerManager::~SkillHandlerManager() {
    ...
    // Delete each skill handler.
    for (int i = 0; i < m_Size; i++) {
        SAFE_DELETE(m_SkillHandlers[i]);
    }

    // Delete the skill handler array.
