...
//////////////////////////////////////////////////////////////////////////////
// class Item
// Passive items lying on the ground or owned by a creature.
//////////////////////////////////////////////////////////////////////////////

class Item : public Object {
public:
    // Subclassification of Item. Please add every class
    // that derives directly from Item to the list below.
    // Those classes must also override
    // getItemClass().
    enum ItemClass {
        ITEM_CLASS_MOTORCYCLE,          // 0
        ITEM_CLASS_POTION,              // 1
    ...
    enum CreateType {
        CREATE_TYPE_NORMAL,         // ordinary item creation				// 0
        CREATE_TYPE_MONSTER,        // created by a monster				// 1
        CREATE_TYPE_SHOP,           // created in a shop					// 2
        CREATE_TYPE_GAMBLE,         // created through gambling				// 3
        CREATE_TYPE_ENCHANT,        // created through enchanting				// 4
        CREATE_TYPE_GAME,           // created by another game system	// 5
        CREATE_TYPE_CREATE,         // created by a GM command				// 6
        CREATE_TYPE_MALL,           // bought from the web shop mall		// 7
        CREATE_TYPE_PRIZE,          // given out by some other event			// 8
        CREATE_TYPE_MIXING,         // made with a Mixing Forge		// 9
        CREATE_TYPE_SPECIAL,        // the boss's item				// 10
        CREATE_TYPE_TIME_EXTENSION, // item whose time limit is extended for the new-user event	// 11
...
public:
    virtual void create(const string& ownerID, Storage storage, DWORD storageID, BYTE x, BYTE y,
                        ItemID_t itemID = 0) = 0; // itemID=0 <-- create with a specific ItemID.
    ...
    // own methods
    // *CAUTION*
    // ItemType is defined in Item's subclasses.
    // The reason is again to avoid the waste caused by
    // memory alignment.
public:
...
protected:
    ...
    CreateType m_CreateType; // how the item was created
    bool m_bUnique;          // is this a unique item?
    bool m_bQuest;           // is this a quest item?
    bool m_bTrace;           // should this item leave a Trace Log?

    bool m_bTimeLimit; // is this a time-limited item?
    int m_Hour;        // the time limit

    bool m_bOnStore; // is it on display in a personal store?
    Store* m_pStore;
};
