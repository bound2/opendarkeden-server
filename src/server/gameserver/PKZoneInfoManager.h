...
class PKZoneInfo {
public:
    PKZoneInfo(ZoneID_t zoneID, ZoneCoord_t enterX, ZoneCoord_t enterY, ZoneCoord_t resX, ZoneCoord_t resY, Race_t race,
               int pcLimit) {
        m_ZoneID = zoneID;
    ...
    ZoneCoord_t getEnterX() const {
        return m_EnterPosition.x;
    }
    ZoneCoord_t getEnterY() const {
        return m_EnterPosition.y;
    }
    ZoneCoord_t getResurrectX() const {
        return m_ResurrectPosition.x;
    }
    ZoneCoord_t getResurrectY() const {
        return m_ResurrectPosition.y;
    }
    int getPCLimit() const {
        return m_PCLimit;
    }
    int getCurrentPCNum() const {
        return m_CurrentPCNum;
    }

    bool canEnter() const {
        return m_CurrentPCNum < m_PCLimit;
    }
    bool enterZone() {
        if (canEnter())
            m_CurrentPCNum++;
