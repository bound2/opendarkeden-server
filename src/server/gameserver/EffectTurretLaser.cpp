...

{
    ...
    if (!isValidZoneCoord(pZone, Cx, Cy))
        return;
    ...
    for (; itr != oList.end(); ++itr) {
        ...
        if (pCreature->isMonster() || pCreature->isFlag(Effect::EFFECT_CLASS_SIEGE_REINFORCE) ||
            pCreature->isFlag(Effect::EFFECT_CLASS_SIEGE_DEFENDER))
            continue;
