...
string UserGateway::s_UserGatewayTypeToString[MAX_USER_GATEWAY_TYPE] = {
    "IN_NORMAL",   "OUT_NORMAL",     "KICKED",           "I_I_ERROR",         "I_I_DISCONNECT",
    "I_C_ERROR",   "I_C_DISCONNECT", "I_O_ERROR",        "I_O_DISCONNECT",    "I_O_DISCONNECT2",
    "I_EXCEPTION", "ZPM_I_ERROR",    "ZPM_I_DISCONNECT", "ZPM_I_DISCONNECT2", "ZPM_C_PAYPLAY_END",
...
string UserGateway::toString() {
    ...
    for (int i = 0; i < MAX_USER_GATEWAY_TYPE; i++) {
        if (m_UserInfos[i] > 0) {
            ...
            msg << getUserGatewayTypeToString((UserGatewayType)i) << "=" << m_UserInfos[i];
            bPreviousPrint = true;
        }
    }
