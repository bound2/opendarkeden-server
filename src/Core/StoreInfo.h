...
public:
    ...
    BYTE isItemExist() const {
        return m_ItemExist;
    }
    void setItemExist(BYTE exist) {
        m_ItemExist = exist;
    }
...
public:
    ...
    // many characters, so a longer one is cut to the cap.
...
public:
    ...
    const vector<StoreItemInfo>& getItems() const {
        return m_Items;
    }

    StoreItemInfo& getStoreItemInfo(BYTE index) {
        return m_Items[index];
    }
    void makeStoreOutlook(StoreOutlook& outlook) {
        outlook.setOpen(m_Open);
        outlook.setSign(m_Sign);
    }
