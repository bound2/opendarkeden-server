...
void CGExchangeListHandler::execute(CGExchangeList* pPacket, Player* pPlayer) {
    ...
    vector<ExchangeListing> listings =
        ExchangeService::getListings(serverID, pPacket->getPage(), pPacket->getPageSize(), pPacket->getItemClass(),
                                     pPacket->getItemType(), pPacket->getMinPrice(), pPacket->getMaxPrice());
