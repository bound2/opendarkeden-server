...
| R14 | Mentions of the never-defined region macros | 0 | `LC_ALL=C grep -rhE '__(THAILAND\|THIALAND\|CHINA\|CHAINA\|INTERNATIONAL\|NETMARBLE\|TEST)_SERVER__' src --include='*.h' --include='*.cpp' \| wc -l` — nothing defines `__THAILAND_SERVER__` or `__CHINA_SERVER__`, so a block behind one never reached the compiler and its `#ifndef`/`#else` neighbour was the only code the servers ran; a translation-unit-local `#define` arms them again, and one did, giving that TU a `SystemAvailabilitiesManager` with an extra member and a layout no other TU shared. Counted with them: the misspellings `__CHAINA_SERVER__` and `__THIALAND_SERVER__`, which appeared inside the same conditions, and `__INTERNATIONAL_SERVER__`, a dead `#elif` in the same `Encrypter.h` chain. `__NETMARBLE_SERVER__` (a portal build) and `__TEST_SERVER__` (a test build) are defined by nothing either and are counted with them. Comments count too, so a comment about one of these branches states the behaviour instead |
...
| `src/server/gameserver/skill/SkillUtil.cpp` | 6,718 (enforced by `ratchets.sh` R6a) |
