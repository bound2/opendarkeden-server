...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty managers, each registered by
  ...
  > object, its own `main()`, holding it as a local. Next: `g_pSkillInfoManager`
  > (237 files), `g_pZoneGroupManager` (45), `g_pPCFinder` (44) and
  > `g_pZoneInfoManager` (36), then the sharedserver's own set and the
  > `src/Core` externs; `g_pPacketValidator` and the `g_pGameServer*Manager`
  > twins fit none: each server creates one of its own.
