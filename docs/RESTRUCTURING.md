...
| R3 | Files with inline SQL anywhere outside `database/` | (measure on Phase 3 start) | `grep -rlE 'executeQuery' src --include='*.cpp' \| grep -v 'server/database' \| wc -l` |
| R4 | Packet classes with `execute()` still on the packet | 433 | `grep -rlE 'void execute\(Player' src/Core --include='*.h' \| wc -l` |
| R5 | `__BEGIN_TRY` control-flow macro sites in de-core candidates | (measure on Phase 3 start) | `grep -rE '__BEGIN_TRY' src/server/gameserver --include='*.cpp' \| wc -l` |
...
- [ ] **1.1 Test harness in CMake.** Add GoogleTest (FetchContent or vendored),
...
- [ ] **1.2 Packet round-trip tests with golden byte fixtures.** For each
  ...
  - Note: packets whose `read`/`write` depend on game-state globals need those
    ...
    first candidates for 2.3).

- [ ] **1.3 Wire-layout inventory (`tests/wire-layout.txt`).** A test walks
  every registered `PacketFactory`, records packet ID, name, field order,
  field widths and max size, regenerates the file, and compares it to the
  committed copy (sidecar's `WireFieldInventoryTest` / `wire-fields.txt`
  pattern). A protocol change becomes a reviewable diff in the same commit,
  not a client crash later.
  > **Status:** not started
  - Owner: the inventory test.
...
- [ ] **1.5 Ratchet tests for R1–R5.** Encode the ratchet table as tests with
