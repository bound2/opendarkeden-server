...
| R13 | Duplicated include guard names across `src/**/*.h` | 0 | Each header's **first** `#ifndef` is its guard; the ratchet takes that name from every header (`find -exec awk`, stripping the CR of the CRLF tree) and counts `sort \| uniq -d`. Two headers sharing a guard means whichever one a translation unit reaches first silently swallows the other: the second `#include` expands to nothing and its declarations surface as an unrelated "undeclared identifier" far from the cause, which is how `EffectCallMotorcycle.h` reusing `EffectDecayItem.h`'s guard bit the `Zone` split. 16→0: eighteen headers took a name derived from their path, per-server twins (`sharedserver/GameServerInfo.h` → `__SHARED_SERVER_GAME_SERVER_INFO_H__`) included, because which headers share a binary changes. Not measured on every `^#ifndef` line — feature macros are tested that way too (`__OLD_GUILD_WAR__` in four headers), so that count could never reach zero |
| R14 | Mentions of the never-defined region macros | 0 | `LC_ALL=C grep -rhE '__(THAILAND\|THIALAND\|CHINA\|CHAINA\|INTERNATIONAL)_SERVER__' src --include='*.h' --include='*.cpp' \| wc -l` — nothing defines `__THAILAND_SERVER__` or `__CHINA_SERVER__`, so a block behind one never reached the compiler and its `#ifndef`/`#else` neighbour was the only code the servers ran; a translation-unit-local `#define` arms them again, and one did, giving that TU a `SystemAvailabilitiesManager` with an extra member and a layout no other TU shared. Counted with them: the misspellings `__CHAINA_SERVER__` and `__THIALAND_SERVER__`, which appeared inside the same conditions, and `__INTERNATIONAL_SERVER__`, a dead `#elif` in the same `Encrypter.h` chain. Comments count too, so a comment about one of these branches states the behaviour instead |
...
| File | Baseline lines |
|------|---------------:|
| `src/server/gameserver/Zone.cpp` | 1,474 (was 9,350 before the 4.2 extractions; enforced by `ratchets.sh` R6g) |
| `src/server/gameserver/skill/SkillUtil.cpp` | 6,722 (enforced by `ratchets.sh` R6a) |
| `src/server/gameserver/InitAllStat.cpp` | 4,799 (was 4,949 before the 3.3 bonus-formula extraction; enforced by `ratchets.sh` R6b) |
...
| `src/server/gameserver/Slayer.cpp` | 3,605 (was 4,046 before the 4.3 hoists; enforced by `ratchets.sh` R6h) |
| `src/server/gameserver/Vampire.cpp` | 2,324 (was 2,783 before the 4.3 hoists; enforced by `ratchets.sh` R6i) |
| `src/server/gameserver/Ousters.cpp` | 2,187 (was 2,548 before the 4.3 hoists; enforced by `ratchets.sh` R6j) |
...
| `src/server/gameserver/skill/HitRoll.cpp` | 736 (not a god file — an extraction-target pin, locked in with its 3.3 extraction; enforced by `ratchets.sh` R6c) |
...
- [x] **4.2 Split `Zone.cpp`** (9,263 lines) by concern: movement, broadcast,
  ...
  > in `ZoneInternal.h`.
  > `Zone.cpp` 9,350 → 1,474, pinned by `ratchets.sh` R6g: the
