...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to sixty-six managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 8.
  ...
  > `LogData` with it. Next: the mofus pair, the two that remain of the
  > gameserver's own set (4 files each) — then the externs the binaries
  > share out of `src/Core` and `src/server` —
