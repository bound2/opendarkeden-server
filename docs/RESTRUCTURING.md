...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to sixty-three managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 11.
  ...
  > `LogData` with it. Next: the five that remain of the gameserver's
  > own set, largest first — `g_pHolyLandManager` (13 files),
