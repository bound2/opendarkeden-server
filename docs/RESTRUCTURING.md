...
| `src/server/gameserver/Ousters.cpp` | 1,959 (was 2,548 before the 4.3 hoists, 2,117 before the commented-out code went; enforced by `ratchets.sh` R6j) |
...
- [ ] **3.2 Repository extraction.** Pull inline `executeQuery` SQL out of
  ...
  > `item/SubInventory.{cpp,h}` and `war/SubInventoryItemPosition.{cpp,h}`
  > go the same way: neither is in any CMakeLists (the war file sits in a
  > commented-out entry), neither would compile — `Item::ITEM_CLASS_SUB_INVENTORY`
  > and `POS_TYPE_SUB_INVENTORY` are declared nowhere, `SubInventoryFactory`
  > is registered nowhere, and no file outside the pair includes either header. The
  > `GEAR_SUB_INVENTORY` spec row in `MySQLItemObjectRepository.cpp` stays:
  > the repository is data-driven and `SubInventoryObject` /
  > `SubInventoryInfo` are both in `initdb/DARKEDEN.sql`, where the MySQL
  > tier exercises them.
...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 35.
  ...
  > gameserver's own set, largest first — `g_pStringPool` (80 files) and
  > `g_pGuildManager` (79), each a name the sharedserver declares separately,
  > `g_pVariableManager` (79), `g_pItemFactoryManager` (52),
  > `g_pZoneGroupManager` (45), `g_pPCFinder` (44), `g_pCastleInfoManager` (43),
  > `g_pZoneInfoManager` and `g_pWarSystem` (36 each), then the fourteen smaller
  > ones, the sharedserver's own set and the `src/Core` externs;
  > `g_pPacketValidator` and the `g_pGameServer*Manager` twins fit none: each
  > server creates one of its own.
