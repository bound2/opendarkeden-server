...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 19.
  ...
  > gameserver's own set, largest first — `g_pStringPool` (74 files) and
  > `g_pGuildManager` (66), each a name the sharedserver declares
  > separately, then the eleven smaller ones
