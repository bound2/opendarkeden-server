...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty-six managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 18.
  ...
  > `LogData` with it. Next: the rest of the
  > gameserver's own set, largest first — `g_pGuildManager` (66 files), a
  > name the sharedserver declares separately, then the eleven smaller ones,
  > the sharedserver's own six
  > and the externs the binaries share out of `src/Core` and `src/server`
  > — `g_pConfig` (76) and `g_pDatabaseManager` (67) the largest of
  > those; `g_pPacketValidator` and the `g_pGameServer*Manager` twins fit
  > none: each server creates one of its own.
