...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty-nine managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 15.
  ...
  > `LogData` with it. Next: the nine that remain of the gameserver's
  > own set, largest first — `g_pParkingCenter` and
  > `g_pLevelWarZoneInfoManager` (15 files each), `g_pLoginServerManager`
  > and `g_pShrineInfoManager` (14), `g_pHolyLandManager` (13),
