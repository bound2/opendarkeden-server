...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to sixty-five managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 9.
  ...
  > `LogData` with it. Next: the three that remain of the gameserver's
  > own set — `g_pSweeperBonusManager` (12 files) and the mofus pair
  > (4 each) — then the externs the binaries share out of `src/Core` and
  > `src/server` —
