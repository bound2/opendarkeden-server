...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 26.
  ...
  > and `g_pVariableManager` (79), then the eleven smaller ones, the
  > sharedserver's own six and the externs the binaries share out of
