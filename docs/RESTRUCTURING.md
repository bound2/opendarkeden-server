...
| R17 | Source lines carrying a non-ASCII byte | 581 | `LC_ALL=C grep -rhE $'[^\x01-\x7f]' src --include='*.h' --include='*.cpp' \| wc -l` - the tree's code language is English, and these are the lines a reader cannot read. The legacy text survived in three states: readable Korean, mojibake (EUC-KR/CP949 bytes decoded as Latin-1 and re-encoded as UTF-8), and U+FFFD runs where the text is gone and only the adjacent code says what it meant. Comments are translated tree by tree, and the last of them -- the gameserver's `skill`, `quest` and `item` trees -- is done, so no comment carries legacy text any more. String literals are a pass of their own: changing one changes what the server says, not how the source reads, so 578 of the 581 lines this count still holds are string literals, 88 of them in the gameserver's `skill` (14), `quest` (72) and `item` (2) trees and 490 in the rest of the tree; the other three are English comments in `src/server/repository` that spell an em dash or an arrow. The byte class is R12's, so the CR of a CRLF working tree stays out of the count |
| R18 | Commented-out code lines inside multi-line `/* */` blocks | 3301 | A character-level perl scan (see `tests/ratchet/ratchets.sh`) over every `src/**/*.{cpp,h}`: lines inside a multi-line `/* */` comment that read as a statement — ending in `;`, `{` or `}`, a preprocessor directive, opening with a control keyword or type name, or an identifier followed by `(`. Switched-off code names deleted globals and signatures that changed, and reads as if it described the code beside it. The same test over `//` lines called 13 of 50 sampled prose lines code — section banners, end-of-block markers, sentences opening with `delete` — so only `/* */` blocks are counted; `//` commented-out code is removed without being measured. 10639→5281 on 2026-09-17. 5,279→3,306 on 2026-09-18, with `src/Core`, `src/server/database`, the login and shared servers, the files directly under `src/server` and the gameserver’s `handler/`, `gm/`, `war/`, `mofus/`, `mission/`, `couple/`, `ctf/`, `packetfill/`, `repository/` and `exchange/` trees |
...
| `src/server/gameserver/Slayer.cpp` | 3,589 (was 4,046 before the 4.3 hoists; enforced by `ratchets.sh` R6h) |
| `src/server/gameserver/Vampire.cpp` | 2,308 (was 2,783 before the 4.3 hoists; enforced by `ratchets.sh` R6i) |
| `src/server/gameserver/Ousters.cpp` | 2,183 (was 2,548 before the 4.3 hoists; enforced by `ratchets.sh` R6j) |
...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to forty-eight managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 38.
  ...
  > object, its own `main()`, holding it as a local. Next: `g_pItemInfoManager`
  > and `g_pFlagManager`; `g_pPacketValidator` and the `g_pGameServer*Manager`
...
- [ ] **4.3 Race-class cleanup.** `Slayer.cpp`/`Vampire.cpp`/`Ousters.cpp`
  ...
  > What is still written three times is each race's wear, skill-slot,
  > persistence-record and load code; those bodies are character-for-character
  > identical in Vampire and Ousters, but only after substituting a type the
  > two do not share — a class-scoped `WearPart` enum whose members differ in
  > name and value, an independent `VampireSkillSlot`/`OustersSkillSlot`
  > (neither derives from `SkillSlot`), or a
  > `VampireExpsRecord`/`OustersExpsRecord` with its own repository method —
  > so the next shrink is a reconciliation of one of those types, not another
  > hoist. Line counts pinned by `ratchets.sh` R6h/R6i/R6j, `__BEGIN_TRY`
  > sites by R5.
