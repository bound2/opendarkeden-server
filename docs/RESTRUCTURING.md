...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty-eight managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 16.
  ...
  > `LogData` with it. Next: the ten that remain of the gameserver's
  > own set, largest first — `g_pSharedServerManager` (16 files),
  > `g_pParkingCenter` and `g_pLevelWarZoneInfoManager` (15 each),
  > `g_pLoginServerManager` and `g_pShrineInfoManager` (14),
  > `g_pHolyLandManager` (13), `g_pResurrectLocationManager` and
  > `g_pSweeperBonusManager` (12) and the mofus pair (4 each) — then the
