...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty-five managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 27.
  ...
  > `g_pVariableManager` (79) and `g_pPKZoneInfoManager` (19), then the
  > eleven smaller ones, the sharedserver's own set and the `src/Core`
  > externs; `g_pPacketValidator` and the `g_pGameServer*Manager` twins fit
