...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty-four managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 28.
  ...
  > `g_pVariableManager` (79), `g_pOptionInfoManager` (22) and
  > `g_pPKZoneInfoManager` (19), then the eleven smaller ones, the
  > sharedserver's own set and the `src/Core` externs; `g_pPacketValidator`
  > and the `g_pGameServer*Manager` twins fit none: each server creates one
  > of its own.
