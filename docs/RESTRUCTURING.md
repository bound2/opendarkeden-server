...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 20.
  ...
  > and `g_pVariableManager` (79), then the eleven smaller ones and the
  > externs the binaries share out of
  > `src/Core` and `src/server` — `g_pConfig` (76) and `g_pDatabaseManager`
  > (67) the largest of those; `g_pPacketValidator` and the
  > `g_pGameServer*Manager` twins fit none: each server creates one of its
  > own.
