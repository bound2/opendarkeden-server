...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to fifty-seven managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 17.
  ...
  > `LogData` with it. Next: the eleven that remain of the gameserver's
  > own set, largest first — `g_pSkillHandlerManager` (17 files),
  > `g_pSharedServerManager` (16), `g_pParkingCenter` and
  ...
  > `g_pConfig` (76) and `g_pDatabaseManager` (67) the largest of those,
  > then `g_pPacketFactoryManager` and `g_pGameWorldInfoManager` (10 each),
  > `g_pPacketValidator` (7) and ServerCore's `g_pGameServerInfoManager`;
  > the validator and the factory manager fit none of the rules above: each
