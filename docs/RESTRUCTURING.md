...
- [ ] **3.5 Globals → context (long tail).** No big-bang DI. Introduce a
  ...
  > registry of non-owning pointers to sixty-one managers, each registered by
  ...
  > linked, which the forward-declaration-only headers are for. R1: 325 → 13.
  ...
  > `LogData` with it. Next: the seven that remain of the gameserver's
  > own set, largest first — `g_pLoginServerManager` and
  > `g_pShrineInfoManager` (14 files each), `g_pHolyLandManager` (13),
